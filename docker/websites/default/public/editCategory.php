...
$url = "editCategory.php?id=".$current_a_id;
...
$stmt = $pdo->prepare("SELECT * FROM categories WHERE id = '$cat_id_edit'");
$stmt->execute();
//$categories = $stmt->fetch();
while ($article = $stmt->fetch()) {
    echo "<label>Edit category</label>";
   
    echo '<input type="text" name="new_cat_namee" value="'.$article['category_name'].'"/>';
/*   
...
if (isset($_POST['Update'])) {
  $new_cat_namee = $_POST['new_cat_namee'];
  ...
  //$new_category = $_POST['category'];
    ...
    $stmt = $pdo->prepare("UPDATE categories SET category_name = '$new_cat_namee', date_added ='$new_date' WHERE id = '$cat_id_edit';");
